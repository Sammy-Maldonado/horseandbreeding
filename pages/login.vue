<template>
  <main
    class="relative flex flex-1 flex-col overflow-hidden px-4 py-4 sm:px-6 lg:px-8"
  >
    <div
    ...
    >
      ...
      <form action="/login" class="w-full max-w-sm" @submit.prevent="logIn">
        ...
        <div class="mb-6">
          ...
          <input
            ...
            type="email"
            id="email"
            class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-xs sm:text-sm focus:outline-hidden placeholder:text-slate-400 focus:ring-2 focus:ring-sky-500 ring-1 ring-slate-200"
            required
          />
        </div>
        <div class="mb-6">
        ...
        <button
          type="submit"
          class="inline-flex justify-center rounded-lg text-sm font-semibold py-2.5 px-4 bg-sky-700 ring-sky-600/20 text-white hover:bg-slate-700 w-full"
        >
...
const logIn = async () => {
  try {
    ...
    if (authData && authData.statusCode === 200) {
      ...
      message.value = " Redirecting to the main page...";
      isError.value = 0; // Set error state
      // Navigate to /demo after a short delay
      setTimeout(() => {
        router.push("/");
      }, 1500); // Delay for 1.5 seconds for user to see the message
    } else {
